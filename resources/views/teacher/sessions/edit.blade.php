<x-layout>
    <header class="relative bg-gray-800 after:pointer-events-none after:absolute after:inset-x-0 after:inset-y-0 after:border-y after:border-white/10">
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <h1 class="text-3xl font-bold tracking-tight text-white">Edit Live Session</h1>
        </div>
    </header>
    <main>
            ...
            <div class="bg-gray-800 rounded-lg shadow-md p-6">
                <form method="POST" action="/teacher/sessions/{{ $session->id }}">
                    ...
                    <x-form-field>
                        ...
                        <x-form-input type="text" name="title" id="title" value="{{ old('title', $session->title) }}" required/>
                        <x-form-error name="title"/>
                    </x-form-field>
                    <x-form-field>
                        <x-form-label for="description">Description</x-form-label>
                        <x-form-textarea name="description" id="description" rows="5">{{ old('description', $session->description) }}</x-form-textarea>
                        <x-form-error name="description"/>
                    </x-form-field>
                    <x-form-field>
                        <x-form-label for="meeting_url">Meeting URL (Zoom, Google Meet, etc.)</x-form-label>
                        <x-form-input type="url" name="meeting_url" id="meeting_url" value="{{ old('meeting_url', $session->meeting_url) }}" required placeholder="https://..."/>
                    ...
                    <div class="flex justify-end mt-6">
                        ...
                        <x-form-button>Update Session</x-form-button>
                    </div>
                </form>
            </div>
        </div>
