<x-layout>
    <header class="relative bg-gray-800 after:pointer-events-none after:absolute after:inset-x-0 after:inset-y-0 after:border-y after:border-white/10">
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <h1 class="text-3xl font-bold tracking-tight text-white">Create New Speaking Phrase</h1>
        </div>
    </header>
    <main>
        <div class="mx-auto max-w-3xl px-4 py-6 sm:px-6 lg:px-8">
            <div class="bg-gray-800 rounded-lg shadow-md p-6">
                <form method="POST" action="/admin/phrases">
                    @csrf
                    <x-form-field>
                        <x-form-label for="phrase">Phrase</x-form-label>
                        <x-form-textarea name="phrase" id="phrase" rows="3" required>{{ old('phrase') }}</x-form-textarea>
                        <x-form-error name="phrase"/>
                    </x-form-field>
                    <x-form-field>
                        <x-form-label for="category">Category</x-form-label>
                        <x-form-input type="text" name="category" id="category" value="{{ old('category', 'general') }}" required/>
                    ...
                    <div class="flex justify-end mt-6">
                        ...
                        <x-form-button>Create Phrase</x-form-button>
                    </div>
                </form>
            </div>
        </div>
